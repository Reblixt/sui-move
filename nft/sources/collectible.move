...
/// [DEPRECATED]
/// Please don't use this package. It's a part of the Kiosk testnet package and
/// was published by mistake. Kept for backwards compatibility and will be
/// removed after the first testnet wipe.
///
/// THIS MODULE IS NOT A PART OF THE KIOSK MAINNET PACKAGE
///
///
...
module nft::collectible {
    use std::{option::{Self, Option}, string::String, vector as vec};
    use sui::{
    ...
    public struct CollectionCap<T: store> has key, store {
        id: UID,
    ...
    public fun mint<T: store>(
        ...
        assert!(
            option::is_none(&cap.max_supply) || *option::borrow(&cap.max_supply) > cap.minted,
            ECapReached,
        );
    ...
    public fun borrow_publisher<T: store>(self: &mut CollectionCap<T>): (Publisher, Borrow) {
